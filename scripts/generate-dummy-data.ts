...
function generateDummyMatch(): Match {
  ...
  if (Math.random() > 0.2 && ARCHETYPES_BY_CLASS[playerClass]) {
    // 80% chance to have an archetype
    playerArchetype = getRandomElement(ARCHETYPES_BY_CLASS[playerClass])
  }

  let opponentArchetype: string | undefined = undefined
  if (Math.random() > 0.2 && ARCHETYPES_BY_CLASS[opponentClass]) {
    // 80% chance to have an archetype
...
function insertDummyData(numMatches: number) {
  try {
    ...
    const insertArchetypeStmt = db.prepare(
      'INSERT OR IGNORE INTO archetypes (name, class_name) VALUES (?, ?)'
    )
...
if (isNaN(numToGenerate) || numToGenerate <= 0) {
  console.error(
    '生成するマッチ数を正しく指定してください (例: ts-node scripts/generate-dummy-data.ts 50)'
  )
  process.exit(1)
}
